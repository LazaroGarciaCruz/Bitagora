...
public extension EasyTipView {
    ...
    public func show(animated: Bool = true, forView view: UIView, withinSuperview superview: UIView? = nil) {
        ...
        //tap.delegate = self
        contenido.addGestureRecognizer(tap)
...
open class EasyTipView: UIView {
    ...
    fileprivate lazy var contentSize: CGSize = {
        ...
        /*var contentSize = CGSize(width: self.contenido.w + self.preferences.positioning.textHInset * 2 + self.preferences.positioning.bubbleHInset * 2, height: self.contenido.h + self.preferences.positioning.textVInset * 2 + self.preferences.positioning.bubbleVInset * 2 + self.preferences.drawing.arrowHeight)*/
        var contentSize = CGSize(width: self.contenido.w, height: self.contenido.h)
    ...
    public init (contenido: UIView, preferences: Preferences = EasyTipView.globalPreferences, delegate: EasyTipViewDelegate? = nil){
    ...
    fileprivate func drawView(_ bubbleFrame: CGRect, context : CGContext) {
        
        let viewRect = CGRect(x: bubbleFrame.origin.x + (bubbleFrame.size.width - contenido.w) / 2, y: bubbleFrame.origin.y + (bubbleFrame.size.height - contenido.h) / 2, width: contenido.w, height: contenido.h)
        
        contenido.draw(viewRect)
    ...
    override open func draw(_ rect: CGRect) {
        ...
        drawView(bubbleFrame, context: context)
        
        context.restoreGState()
        
        contenido.x = bubbleXOrigin
        contenido.y = bubbleYOrigin
        contenido.w = bubbleWidth
        contenido.h = bubbleHeight
        contenido.cornerRadius = preferences.drawing.cornerRadius
        
        self.addSubview(contenido)
